    ...
    async function load() {
      ...
      const [{ data: profileData }, { data: hintsData }] = await Promise.all([
        ...
        Promise.all([
          ...
          supabase.from("hints").select("id, title, image_url, numeric_price, currency, retailer, url, starred, occasions, size, size_type").eq("user_id", userId).eq("is_private", false).neq("starred", true).order("created_at", { ascending: false }).limit(3),
