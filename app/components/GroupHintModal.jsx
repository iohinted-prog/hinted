...
export default function GroupHintModal({ hint, recipientUserId, recipientName, currentUserId, onClose }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Check if group hint already exists for this hint
      const { data: existing } = await supabase
        .from("group_hints")
        .select("*, group_hint_members(*, profiles(full_name, avatar_url))")
        .eq("hint_id", hint.id)
  ...
  async function handleSend() {
    ...
    // Send feed notifications to each invited contact
    for (const userId of selected) {
      await supabase.from("feed_items").insert({
        ...
        metadata: {
          ...
          hint_image: hint.image_url || hint.image || "",
      ...
      <div className="w-full max-w-[480px] rounded-t-[28px] sm:rounded-[28px] bg-[#fffaf7] border border-[#efdcd2] shadow-xl overflow-hidden flex flex-col" style={{ maxHeight: "88dvh" }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#f2e5de] shrink-0">
          <div>
            ...
            <p className="text-[12px] text-slate-400 mt-0.5">for {hint.title || "this hint"} → {recipientName}</p>
          </div>
          ...
          {loading ? (
            ...
            <>
                  ...
                  {members.map(m => (
                    <div key={m.id} className="flex items-center gap-3 py-2">
                      {m.profiles?.avatar_url
                        ? <img src={m.profiles.avatar_url} className="h-9 w-9 rounded-full object-cover" />
                        : <div className="h-9 w-9 rounded-full bg-gradient-to-b from-[#efcdbf] to-[#bb8168] flex items-center justify-center text-[11px] font-bold text-white">{m.profiles?.full_name?.[0] || "?"}</div>
                      }
              ...
              {availableContacts.length > 0 && (
                <div>
                  <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-2">Invite contacts</p>
                  {availableContacts.map(c => (
                    <div key={c.profile_id} className="flex items-center gap-3 py-2 cursor-pointer" onClick={() => toggleContact(c.profile_id)}>
                      {c.avatar_url
                        ? <img src={c.avatar_url} className="h-9 w-9 rounded-full object-cover" />
                        : <div className="h-9 w-9 rounded-full bg-gradient-to-b from-[#efcdbf] to-[#bb8168] flex items-center justify-center text-[11px] font-bold text-white">{c.name?.[0] || "?"}</div>
                      }
                      <p className="text-[13px] font-semibold text-slate-900 flex-1">{c.name}</p>
                      <div className={"h-5 w-5 rounded-full border-2 flex items-center justify-center " + (selected.includes(c.profile_id) ? "border-[#ff875d] bg-[#ff875d]" : "border-slate-300")}>
              ...
              {done && (
                <div className="mt-3 rounded-[14px] bg-[#edf6eb] px-4 py-3 text-[13px] font-semibold text-[#4a7a3a]">
        ...
        {selected.length > 0 && (
          <div className="px-4 pb-5 pt-2 border-t border-[#f2e5de] shrink-0">
            <button type="button" disabled={sending} onClick={handleSend}
              ...
              {sending ? "Sending..." : `Invite ${selected.length} contact${selected.length > 1 ? "s" : ""}`}
            </button>
          </div>
        )}
      </div>
