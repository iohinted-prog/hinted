...
function detectBlock(bodyText, titleTag, h1, status) {
  ...
  if (
    ...
    (text.includes("cloudflare") || text.includes("security service") || text.includes("you have been blocked"))
  ) {
    return {
      blocked: true,
...
async function scrapeUrl(inputUrl, preferredCurrency = "GBP") {
  ...
  const status = 200; // We already validated == 200 in fetchHtml
  ...
  if (isAmazon && title && rule.cleanTitle) title = rule.cleanTitle(title);
  ...
  const confidence =
    hasStrongImage && hasTitle && hasPrice ? "high" : hasTitle && (hasStrongImage || hasPrice) ? "medium" : "low";
    ...
    try {
      ...
      if (scrapeError?.status === 503) {
        return NextResponse.json({
          ...
          source: "blocked",
          debug: {
            error: scrapeError?.message || "Amazon 503 error",
            status: 503,
          },
        }, { status: 200 });
      }

      if (scrapeError?.status === 403) {
        return NextResponse.json({
          url: inputUrl,
          title: "",
          description: "",
          ...
          source: "blocked",
          debug: {
            error: scrapeError?.message || "403 blocked",
            status: 403,
          },
        }, { status: 200 });
      }
