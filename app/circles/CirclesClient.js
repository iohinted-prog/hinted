...
function buildContactRecord({ id, name, email, relationship }) {
  const safeName = (name || email || "Unnamed contact").trim();
  const safeRelationship = relationship || "Friend";

  return {
    id,
    name: safeName,
    role: safeRelationship,
    note: "Added from linked contacts",
    initials: getInitials(safeName),
    colors: getRelationshipGradient(safeRelationship),
    email: (email || "").trim(),
    phone: "",
    birthday: "",
  };
...
function AddContactModal({ open, onClose, onSave, supabase }) {
  ...
  async function handleSave() {
    ...
    const primaryRelationship = selectedRelationships[0] || "Friend";
...
export default function CirclesClient() {
  ...
  const resetCircleForm = () => {
    ...
    setSelectedPeople([]);
    setSelectedHintContactId(contacts[0]?.id ?? null);
    setLinkPreview(null);
    setForm({
      eventTitle: calendarEvents[0].title,
      eventDate: calendarEvents[0].date,
