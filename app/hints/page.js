...
function getPriceTier(price) {
  if (price >= 200) {
    return {
      pill: "bg-[#2f3b2d] text-white",
      label: "Big gift",
      sizeClass: "min-h-[360px]",
    };
  }

  if (price >= 100) {
    return {
      pill: "bg-[#fff2ea] text-[#df7a59]",
      label: "Premium",
  ...
  if (price >= 50) {
    return {
      pill: "bg-[#f4f0ff] text-[#7b5fbc]",
      label: "Mid-range",
      sizeClass: "min-h-[260px]",
    };
  }

  return {
    pill: "bg-[#f3f6ef] text-[#5f7c4f]",
    label: "Smaller spend",
    sizeClass: "min-h-[220px]",
  };
}

function HintCard({ hint }) {
  const tier = getPriceTier(hint.price);

  return (
    <article
      className={`group flex break-inside-avoid flex-col overflow-hidden rounded-[28px] border ${
        hint.starred
          ? "border-[#efcfbf] bg-[#fffaf7] shadow-[0_14px_30px_rgba(184,122,92,0.12)]"
          : "border-[#f0dfd6] bg-white shadow-sm"
      } transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_16px_36px_rgba(171,114,84,0.14)] ${tier.sizeClass}`}
    >
      <div className={`relative h-[140px] w-full bg-gradient-to-br ${hint.image} sm:h-[170px]`}>
        <div className="flex items-start justify-between p-4">
          <span className={`rounded-full px-3 py-1 text-[11px] font-semibold ${tier.pill}`}>
            {tier.label}
          </span>
      ...
      <div className="mx-auto max-w-[1440px] px-5 py-8 md:px-8">
            ...
            <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr] xl:items-end">
              <div>
                ...
                <p className="mt-3 max-w-[700px] text-[15px] leading-7 text-slate-600">
                  Save gift ideas from anywhere with a single link, then let the board organise them into a clean visual collection where the bigger spends naturally stand out.
                </p>
              </div>

              <div className="rounded-[24px] border border-[#efdfd7] bg-[#fffdfa] p-4">
                <label htmlFor="hint-link" className="block text-[12px] font-semibold uppercase tracking-[0.14em] text-slate-400">
                  Paste a link
                </label>
                ...
                <p className="mt-3 text-[12px] leading-6 text-slate-500">
                  Paste a product, experience, or wishlist link and Hinted can turn it into a clean card on your board.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="mt-8">
          ...
          <div className="columns-1 gap-4 sm:columns-2 lg:columns-3 xl:columns-4">
            {hints.map((hint) => (
              <div key={hint.id} className="mb-4">
                <HintCard hint={hint} />
              </div>
            ))}
          </div>
